...
Why did this happen ? Nmap tried to lookup a DNS name for the target IP before scanning it but the Host-only network has no DNS server to answer that request, which is what cause the "hang"or unresponsiveness.
